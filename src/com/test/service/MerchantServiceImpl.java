...
import com.test.dao.MerchantDAOImpl;
...
public class MerchantServiceImpl implements MerchantService{
  ...
  public JsonData select(Merchant obj) {
    Connection conn = DButil.getConnection();//数据库连接
    MerchantDAOImpl bo = new MerchantDAOImpl(conn);//业务层对象，用于调用DAO层方法
    try{
      ...
      conn.commit();
      success = true; 
      msg = "http://localhost:8080/test/html/merchant/main.jsp";
   ...
   */
  public JsonData insert(Merchant obj){
    Connection conn = DButil.getConnection();//数据库连接
    Merchant merBO = new Merchant(conn);//业务层对象，用于调用DAO层方法
    //MerchantDAOImpl userBO = new MerchantDAOImpl(conn);//业务层对象，用于调用DAO层方法
    try{
    	//添加Merchant账号
        String name = obj.getName();
        String accout = obj.getAccout();
        String password = obj.getPassword();
        String status = "启用";//默认
        String imgURL = "/test/src/img/default/merchant-default.png";//默认图片
        //Util util = new Util();
        //String pw = util.EncoderByMd5("123456");//把默认密码123456加密
        
        Merchant mer = new Merchant();
        mer.setName(name);
        mer.setAccout(accout);
        mer.setPassword(password);
        mer.setStatus(status);
        mer.setImgURL(imgURL);
        
        int i = merBO.insert(mer);//i为影响的行数
        conn.commit();
        success = true; 
        msg = "成功添加第["+i+"]条记录";
        jd = new JsonData(success,msg);
   ...
   */
  public JsonData update(Merchant obj){
    Connection conn = DButil.getConnection();//数据库连接
    MerchantDAOImpl bo = new MerchantDAOImpl(conn);//业务层对象，用于调用DAO层方法
    try{
   ...
   */
  public JsonData delete(Merchant obj){
    Connection conn = DButil.getConnection();//数据库连接
    MerchantDAOImpl stuBO = new MerchantDAOImpl(conn);//业务层对象，用于调用DAO层方法
    //MerchantDAOImpl userBO = new MerchantDAOImpl(conn);//业务层对象，用于调用DAO层方法
    try{
      //根据id查询goods表中记录
      ArrayList<Merchant> rows = new ArrayList<Merchant>();
