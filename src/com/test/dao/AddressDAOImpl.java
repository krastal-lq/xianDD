...
public class AddressDAOImpl implements AddressDAO {
	   ...
	   * 查询记录
	   */
	  public ArrayList<Address> select(Address pojo) throws SQLException {
	    try{
	      String sql = "select "
	                 + "  address.id as id, "
	                 + "  user.name as name, "
	                 ...
	                 + "  user.defaulting as defaulting, "
	                 + "from user "
	                 + "  inner join user on address.userId = user.id "
	                 + "where 1=1 ";
	      // 查询
	      ...
	      while(rs.next()){
	        ...
	        po.setDefaulting(rs.getString("defaulting"));
	        //将以上全部信息添加到集合
	        al.add(po);
	      }
	  ...
	  public int insert(Address pojo) throws SQLException {
	    try{
	      ...
	      pst.setString(4, pojo.getDefaulting());
	      //System.out.println(sql);
	      pst.executeUpdate();
